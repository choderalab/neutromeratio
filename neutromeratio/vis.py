...
def generate_nglview_object(traj:md.Trajectory, radius:int=0.0, donor_hydrogen_idx:int=-1, acceptor_hydrogen_idx:int=-1, mark_residues=[]) -> nglview.NGLWidget:
    """
    ...
    if radius > 0.0:
        view.shape.add_sphere([radius/2, radius/2, radius/2], [0,0,1], (radius)/2)
