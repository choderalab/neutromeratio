...
class LangevinDynamics(object):
    ...
    def check_for_restart_condition(self, ensemble_bias, n_steps):
        ...
        ensemble_bias = np.array(
            [e.value_in_unit(unit.kilocalories_per_mole) for e in ensemble_bias]
        )
        ...
        if nr_of_snapshots_with_ensemble_bias > total_length / 5:
            if np.count_nonzero(ensemble_bias[-10:]) == 0:
                ...
                logger.warning(f"################################################")
                logger.warning(
                    f"Nr of snapshots with ensemble_bias: {nr_of_snapshots_with_ensemble_bias}"
                )
                logger.warning(f"Resetting simulation.")
                logger.warning(
                    f"If this happens repeatedly the simulation might not be possible without adventure mode."
                )
                logger.warning(f"################################################")
                return True  # restart
    ...
    def run_dynamics(
        ...
        sigma_v = (
            np.array([unit.sqrt(kB * temperature / m) / speed_unit for m in masses])
            * speed_unit
        )
        ...
        x = (
            np.array(x0.value_in_unit(distance_unit)) * distance_unit
        )  # x.shape =[N_atoms][3]
        v = (
            np.array(v0.value_in_unit(speed_unit)) * speed_unit
        )  # v.shape = [N_atoms][3]
        ...
        for _ in trange:
            ...
            x[0] += (
                stepsize * 0.5
            ) * v  # NOTE: x.shape = [1][n_atoms][3], but v.shape = [n_atoms][3]
            # o
...
def use_precalculated_md_and_performe_mc(
    top: str, trajs: list, hydrogen_movers: list, mc_every_nth_frame: int
):
    """
