...
class Tautomer(object):
    ...
    def __init__(
        ...
        logger.debug(
            f"Nr of conformations generated per tautomer: {nr_of_conformations}"
        )
        ...
        initial_state_ani_input = self._from_mol_to_ani_input(
            self.initial_state_mol, enforceChirality
        )
        ...
        self.initial_state_ligand_topology: md.Topology = initial_state_ani_input[
            "ligand_topology"
        ]
        self.initial_state_ase_mol: Atoms = initial_state_ani_input["ase_mol"]

        final_state_ani_input = self._from_mol_to_ani_input(
            self.final_state_mol, enforceChirality
        )
    ...
    def initial_state_ligand_degeneracy(self):
        ...
        graph_matcher = GraphMatcher(
            self.initial_state_mol_nx, self.initial_state_mol_nx
        )
    ...
    def add_droplet(
        ...
        if not os.path.exists(pdb_filepath):
            ...
            md.Trajectory(coordinates.value_in_unit(unit.nanometer), topology).save_pdb(
                pdb_filepath
            )
            ...
            pdb.positions = np.array(pdb.positions.value_in_unit(unit.nanometer)) + (
                l_in_nanometer / 2
            )
            # add water
            pdb.addSolvent(
            ...
            if residue.is_water:
                for atom in residue.atoms:
                    if str(atom.element.symbol) == "O":
                        ...
                        logger.debug(
                            "Adding restraint to center to {}".format(atom.index)
                        )
    ...
    def add_COM_for_hybrid_ligand(self, center):
        ...
        self.add_COM_restraint(
            sigma=0.2 * unit.angstrom, point=center, atom_idx=idx, atoms=atoms
        )

    def add_COM_restraint(
        self, sigma: unit.Quantity, point, atom_idx: list, atoms: str
    ):
    ...
    def perform_tautomer_transformation(self):
        ...
        # TODO: fix this!
        self._generate_hybrid_structure(
            self.initial_state_ligand_atoms,
            self._initial_state_ligand_coords[0],
    ...
    def _from_mol_to_ani_input(self, mol: Chem.Mol, enforceChirality: bool):
        ...
        mol = self._generate_conformations_from_mol(
            mol, self.nr_of_conformations, enforceChirality
        )
        # generate coord list
        coord_list = []
    ...
    def _generate_conformations_from_mol(
        self, mol: Chem.Mol, nr_of_conformations: int, enforceChirality: bool = True
    ):
        """
        Helper function - does not need to be called directly.
    ...
    def _perform_tautomer_transformation(
        self, m1: Chem.Mol, m2: Chem.Mol, ligand_bonds: list
    ):
        """
        Helper function - does not need to be called directly.
        ...
        for i in range(len(substructure_idx_m1)):
            ...
            if a1.GetSymbol() != "H":
                ...
                if (len(a1_neighbors)) != (len(a2_neighbors)):
                    ...
                    logger.debug(
                        "Index of atom that accepts hydrogen: {}".format(acceptor)
                    )
                    acceptor_count += 1
                    if acceptor_count > 1:
                        raise RuntimeError(
    ...
    def _generate_hybrid_structure(
        self, ligand_atoms: str, ligand_coords, ligand_topology: md.Topology
    ):
        """
        Helper function - does not need to be called directly.
        ...
        hydrogen_mover = MC_Mover(
            self.heavy_atom_hydrogen_donor_idx,
            self.hydrogen_idx,
        ...
        for _ in range(100):
            hybrid_coord = hydrogen_mover._move_hydrogen_to_acceptor_idx(
                ligand_coords, override=False
            )
            energy = energy_function.calculate_energy(
                [hybrid_coord / unit.angstrom] * unit.angstrom
            )
            if energy.energy < min_e:
                min_e = energy.energy
                min_coordinates = hybrid_coord

        self.hybrid_hydrogen_idx_at_lambda_1 = (
            len(hybrid_atoms) - 1
        )  # this is the real hydrogen at lambda 1!
        ...
        self.hybrid_ligand_restraints.append(
            BondFlatBottomRestraint(
                sigma=0.1 * unit.angstrom,
                atom_i_idx=self.hybrid_hydrogen_idx_at_lambda_1,
                atom_j_idx=self.heavy_atom_hydrogen_acceptor_idx,
                atoms=hybrid_atoms,
            )
        )
        ...
        dummy_atom = hybrid_topology.add_atom(
            "H", md.element.hydrogen, hybrid_topology.residue(-1)
        )
        hybrid_topology.add_bond(
            hybrid_topology.atom(self.heavy_atom_hydrogen_acceptor_idx), dummy_atom
        )
    ...
    def generate_mining_minima_structures(
        self, rmsd_threshold: float = 0.1, include_entropy_correction: bool = False
        ...
        for (
            ...
            for conf_id in range(get_nr_of_confs()):
                ...
                for atom in rdkit_mol.GetAtoms():
                    ...
                    new_coords.x = minimized_coords[0][atom.GetIdx()][0].value_in_unit(
                        unit.angstrom
                    )
                    new_coords.y = minimized_coords[0][atom.GetIdx()][1].value_in_unit(
                        unit.angstrom
                    )
                    new_coords.z = minimized_coords[0][atom.GetIdx()][2].value_in_unit(
                        unit.angstrom
                    )
